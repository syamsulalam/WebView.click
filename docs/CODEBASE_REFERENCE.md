...
- `GenerationJobsTable` punya tombol `Export compact` yang menyalin JSON ringkas jobs yang sedang visible, termasuk provider/model, failure stage, readiness/cooldown metadata, copy audit summary, `offeringCopyCoverage`, `offeringCopyMode`, `conversionPagePattern`, `conversionPrimaryAction`, `conversionAudit`, `designIntent`, `designAudit`, persisted `copyOnlyRetryCoverageDelta`, dan derived `copyOnlyRetryChangedDelta` untuk support/debug tanpa membuka drawer satu per satu.
