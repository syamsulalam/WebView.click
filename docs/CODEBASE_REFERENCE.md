...
- Checkout modal memakai flow bertahap dengan tracker `Step X of N`: offer justification -> term/billing -> optional add/edit counts -> add/edit details only if page actions > 0 -> domain mode/check -> email/payment. Page add/edit is optional; if both counters stay 0, the primary action continues directly to domain without a disabled button or separate skip path.
