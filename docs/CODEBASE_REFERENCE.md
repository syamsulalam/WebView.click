...
- `src/App.tsx` lazy-loads top-level public/admin pages with `React.lazy` + `Suspense`, so admin CRM, docs reader dependencies, generated-site renderer, and heavy public preview code do not all land in the initial app chunk.
- Suspense fallback is route-aware via `useLocation()`: `/admin/*` shows a compact admin shell skeleton with sidebar/table blocks, while public/demo/business routes show a lightweight website hero skeleton.
...
- Renderer memakai token warna turunan palette dari `design.themeVariables.colors`: `onPrimary`, `onAccent`, `onSecondary`, `onBackground`, `ink`, `muted`, `accentText`, `icon`, `capsuleBg`, `capsuleBorder`, `capsuleText`, `footerMuted`, dan `footerBorder`. Jika JSON lama belum punya token ini, renderer menghitung tone gelap/terang dari kombinasi primary/accent dan surface, lalu mengecek contrast; jangan fallback ke white/black generik untuk teks, icon, eyebrow, capsule, CTA, atau footer.
- Existing saved site previews mendapat perbaikan warna dari runtime renderer setelah deploy walau JSON lama belum ditulis ulang. Saved JSON baru mendapat token ini saat generate/regenerate/visual refresh/save; screenshot R2 lama dan exported zip lama tetap perlu retake/re-download agar memakai renderer terbaru.
- Saat owner/admin memilih `brand.paletteOptions`, renderer harus memanggil ulang derivasi token warna setelah `primary`/`accent` aktif berubah; jangan hanya override swatch utama karena `onPrimary`, `accentText`, `icon`, `capsule*`, dan `footer*` akan stale.
...
- Footer diberi boundary `data-wv-site-footer` agar tetap memakai palette/font site tetapi terlindung dari efek card/body/image hover yang tidak cocok untuk konteks footer. Muted paragraph, border, social icon background, dan contact/nav links memakai `footerMuted`/`footerBorder` turunan palette, bukan opacity putih/hitam hard-coded.
...
- Saat generate, `/admin/leads` menunggu extraction `paletteOptions` dari foto Places sebelum mengirim `/api/sites/generate`, sehingga site baru lebih konsisten menyimpan pilihan warna untuk action panel/download. Jika hanya ada satu foto/palette, client/scaffold membuat variasi palette deterministik dari warna yang sama agar owner tetap punya lebih dari satu pilihan sebelum download/setup.
...
- Search bar punya filter `Add photos` yang menampilkan generated site dengan `availableImageCount <= 1`. Row seperti ini juga punya badge `Add photos N` dan tombol row icon `Images` untuk one-click `Re-gather Google photos`, yaitu flow `Re-gather Google data + resave` tanpa AI yang mengambil Google Place Details terbaru dan menyimpan ulang site. Setelah selesai, toast membandingkan image count lama -> baru supaya admin langsung tahu apakah Google menambah foto atau manual image enrichment masih dibutuhkan. Jika Google tetap hanya mengembalikan satu foto, row tetap menjadi kandidat manual image enrichment sebelum owner outreach; satu foto membatasi variasi palette dan membuat demo terasa kurang kaya.
...
- Tombol `Refresh visual variation` memanggil `POST /api/sites/:businessId/refresh-visual-variation` untuk mengubah saved font pairing/typography ke stable seeded variant dan mengirim palette tambahan yang diekstrak client-side dari image site jika saved palette options belum punya pilihan ganda. Jika cuma ada satu image/palette, flow membuat variasi palette deterministik dan UI memperingatkan bahwa variasinya tidak akan se-diverse palette dari beberapa foto bisnis. Action ini tidak memanggil AI, tidak mengubah copy, dan tidak mengubah image.
- Filter `Palette missing` memakai summary `paletteOptionCount`, `availableImageCount`, `needsPaletteOptions`, dan `canAutoRepairPaletteOptions` dari `/api/sites`. Saat list `/admin/sites` di-refresh, row dengan beberapa image tapi saved palette option kurang dari dua otomatis menjalankan refresh visual variation secara sequential sampai batas batch kecil. Row satu-image tetap muncul sebagai manual review/warning karena auto-extraction tidak bisa menghasilkan brand palette yang benar-benar berbeda tanpa foto tambahan.
...
- Status badges for actionable Generated Sites issues are clickable: `Fix N img` runs service-card image repair, `Add photos N` re-gathers Google photos/data without AI, `Palette N/2` refreshes visual palette options, and `Repaired {date}` can re-run image repair/audit. Date badges use safe formatting so old invalid timestamps do not render as `Invalid Date`. Rows with any photo/image/palette issue also show one compact `Fix row issues` action that runs the relevant steps in order: Google photo re-gather, service-card image repair, then visual palette backfill, skipping clean steps and reporting partial failures in one toast.
...
- Batch `Visual filtered` memakai endpoint single-site `POST /api/sites/:businessId/refresh-visual-variation` secara sequential dan dibatasi 10 row per klik; sebelum POST, client mengambil full site JSON dan mencoba mengekstrak palette dari image site yang belum punya `brand.paletteOptions`. Jika filter `Palette missing` aktif, batch ini hanya menyentuh row yang masih punya kurang dari dua saved palette options. Jika site lama punya beberapa image tapi belum punya beberapa palette options, refresh otomatis menambah options sampai minimal ada pilihan ganda. Jika hanya ada satu image atau canvas diblokir CORS, client/server membuat variasi palette deterministik dari palette existing/fallback. Response toast merangkum jumlah site refreshed, font pairing yang berubah, dan palette set yang bertambah.
...
- Sebelum save, Function juga menjalankan `normalizeSiteColorContrast()` untuk menurunkan semantic color tokens dari palette primary/accent/background. Token ini dipakai renderer/CSS preset untuk teks di surface terang/gelap, eyebrow, feature/capsule icons, capsule border, CTA, dan footer; logic harus mempertahankan tone palette sambil memenuhi contrast, bukan memilih straight white/black sebagai default visual.
- Palette hasil ekstraksi foto dinormalisasi melalui `src/lib/colorPaletteRoles.ts`: warna low-chroma/grey diperlakukan sebagai surface/border/muted candidate, sementara warna yang punya hue nyata diprioritaskan sebagai primary/accent. Helper yang sama juga membuat variasi warm/cool/bold/soft dari satu palette saat hanya ada satu image. `SiteRenderer` menjalankan role-normalization runtime agar site lama ikut membaik setelah deploy, sedangkan `/api/sites/generate`, scaffold fallback, palette extraction admin, dan `refresh-visual-variation` menyimpan urutan palette yang sudah lebih aman untuk site baru/regenerate.
...
- `POST /api/sites/:businessId/refresh-visual-variation` membaca full JSON dari R2/D1, menerapkan `applySeededFontPairing(..., force=true)`, menerima optional `paletteOptions` dari admin client, merge/dedupe option baru ke `brand.paletteOptions`, self-heal minimal dua palette options dari saved palette jika client tidak mengirim cukup option, menulis `meta.lastVisualVariationAt`, menjalankan `normalizeSiteColorContrast()`, lalu menyimpan ulang JSON/manifest. Endpoint hanya mengubah saved typography, saved palette options/color tokens, dan metadata timestamp visual; copy, images, sourceData, navigation, dan service pages tidak diubah.
