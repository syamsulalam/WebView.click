...
- Tombol `AI fill filtered` menjalankan About/nav fill untuk maksimal 5 row dari hasil filter/search saat ini. Batch berjalan sequential satu site per waktu, dan tiap site tetap memakai chunked generation job terpisah (`chunked-start` lalu `siteCopy`/`offeringCopy`/`finalize`) supaya tidak menggabungkan banyak AI work ke satu request panjang.
