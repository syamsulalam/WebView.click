...
2. **Export HTML:** Export owner memakai `downloadOwnerSiteZip`: clone DOM yang sudah dirender, hapus elemen internal `.hide-in-export`, download gambar yang sedang tampil ke folder `/img` dalam zip, ubah `<img src>` menjadi path relatif seperti `img/namabisnis-hero.jpg`, absolutkan link non-gambar, tambahkan Tailwind CSS/CDN + stylesheet production + inline SVG favicon + JS inline untuk tab navigation/submenu/contact form, lalu zip `index.html`, `sitemap.xml`, `robots.txt`, folder `/img`, dan PDF branded owner guide. Jangan sertakan JSON internal generator di zip owner.
3. **Product/Service Detail Pages:** JSON generator harus menentukan apakah bisnis lebih cocok memakai produk, layanan, atau keduanya. Setiap produk/layanan penting wajib punya halaman detail sendiri dengan overview, manfaat, detail yang termasuk, review relevan bila ada, FAQ, dan CTA, lalu dimasukkan sebagai submenu navbar.
