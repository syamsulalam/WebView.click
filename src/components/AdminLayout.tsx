...
export default function AdminLayout() {
  ...
  const NavContent = () => (
    <div className="flex h-screen bg-gray-50">
      <div className="w-20 bg-white border-r border-gray-200 flex flex-col items-center py-6 gap-6 relative">
        {/* Tooltip trigger wrapper uses group class in tailwind */}
        <Link 
          to="/admin" 
          className={`p-3 rounded-xl transition-colors group relative ${location.pathname === '/admin' ? 'bg-indigo-50 text-indigo-600' : 'text-gray-500 hover:bg-gray-100'}`}
        >
        ...
        <Link 
          to="/admin/leads" 
          className={`p-3 rounded-xl transition-colors group relative ${location.pathname === '/admin/leads' ? 'bg-indigo-50 text-indigo-600' : 'text-gray-500 hover:bg-gray-100'}`}
        >
          <Users size={24} />
          <span className="absolute left-full ml-3 top-1/2 -translate-y-1/2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-[9999] pointer-events-none">
            CRM Leads
  ...
  if (!isSignedIn) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6 flex-col font-sans">
